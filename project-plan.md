...
- **Command:** `npx create-turbo@latest --template=npm` (or `npm`/`yarn` based on your preference).
- **Structure:**
...
- **Prisma Setup Commands (in `apps/api`):**
  - Install Prisma CLI: `npm add -D prisma`
  - Install Prisma Client: `npm add @prisma/client`
...
- **Command:** `npx @nestjs/cli new api --package-manager npm`
- **Prisma Service:**
...
- **Backend (`apps/api` - `WhatsappService`/`MessagesService`):**
  ...
  - **Initial Auto-Send (MVP):** Configure to _automatically send_ the AI reply if `ai_confidence` is above a certain threshold (e.g., > 0.8) and the message is not classified as a `complaint` or `review`. Otherwise, mark it `unhandled`.
